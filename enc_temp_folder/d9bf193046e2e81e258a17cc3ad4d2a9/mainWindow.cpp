...
namespace Calculator
{
	...
	double mainWindow::Calc(double num, String ^s_oper_char)
	{
		double result = 0;
		if(s_oper_char->Contains("√"))
		{
			result = sqrt(num);
			return result;
		}
		else if(s_oper_char->Contains("x²"))
		{
			return 0.0;
		}
		else if(s_oper_char->Contains("⅟"))
		{
			return 0.0;
		}
		else
	...
	System::Void mainWindow::btn_Equal_Click(System::Object ^sender, System::EventArgs ^e)
	{
		...
		else
		{
			if(is_operator_clicked)
			{
				if(special_oper_char != nullptr && !is_MainWindow_Modified)
				{
					num2 = double::Parse(tb_MainCalcText->Text);
					tb_smallWindow->Text = special_oper_char + " " + oper_char + " " + num2.ToString() + " =";
					num1 = Calc(num1, oper_char, num2);
